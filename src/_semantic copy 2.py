...
class Semantics:
    ...
    def visit_RW_CHAN(self, node):
        if len(node.children) == 0:
            param1 = self.visit(node.value[0])
            param2 = self.visit(node.value[1])
            param3 = self.visit(node.value[2])
        else:
            param1 = self.visit(node.children[0])
            param2 = self.visit(node.children[1])
            param3 = self.visit(node.children[2])
        return "RW_C_CHANNEL"
    
    def visit_RW_PRINT(self, node):
        if len(node.children) == 0:
            param = self.visit(node.value[0])
        else:
            param = self.visit(node.children[0])
        if param != 'RW_STRING':
            raise Exception(f"Type error: output should be string, instead got {param }")
        return 'RW_STRING'
    ...
    def visit_DL_SEMICOLON(self, node):
        if len(node.children) == 0:
            param = self.visit(node.value[0])
        else:
            param = self.visit(node.children[0])
    ...
    def visit_DL_LBRACKET(self, node):
        if len(node.children) == 0:
            param = self.visit(node.value[0])
        else:
            param = self.visit(node.children[0])
    ...
    def visit_DL_RBRACKET(self, node):
        if len(node.children) == 0:
            param = self.visit(node.value[0])
        else:
            param = self.visit(node.children[0])
