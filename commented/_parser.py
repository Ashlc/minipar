...
class Parser:
    ...
    def parse_print(self):
        ...
        print(f"Parsed print statement: {expression.node_type}, value: {expression.value}")
        self.eat(en.DL_RPAREN)
    ...
    def parse_factor(self):
        ...
        if token[0] == en.RW_INT:
            ...
            self.eat(en.ID)
            print(f"[parse_factor] Parsed factor: {token[1]}, creating new node with type: {token[0]}")
            return SyntaxNode(en.ID, identifier_token[1])  # Create syntax node with identifier name

        elif token[0] == en.NUM:
            num_token = self.current_token
            self.eat(en.NUM)
            print(f"[parse_factor] Parsed factor: {num_token[1]}, creating new node with type: {num_token[0]}")
            return SyntaxNode(en.NUM, num_token[1])  # Create syntax node with number value
            ...
            return SyntaxNode(en.STRING_LITERAL, string_token[1])  # Create syntax node with string value
